            ...
            <div class="col-span-12 lg:col-span-6 xl:col-span-3">
                <div class="card mb-0">
                    <div class="flex justify-between mb-4">
                        ...
                        <div class="flex items-center justify-center bg-blue-100 dark:bg-blue-400/10 rounded-border" style="width: 2.5rem; height: 2.5rem">
                            <!-- <i class="pi pi-shopping-cart text-blue-500 !text-xl"></i> -->
                             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><title>water-percent</title><path d="M12,3.25C12,3.25 6,10 6,14C6,17.32 8.69,20 12,20A6,6 0 0,0 18,14C18,10 12,3.25 12,3.25M14.47,9.97L15.53,11.03L9.53,17.03L8.47,15.97M9.75,10A1.25,1.25 0 0,1 11,11.25A1.25,1.25 0 0,1 9.75,12.5A1.25,1.25 0 0,1 8.5,11.25A1.25,1.25 0 0,1 9.75,10M14.25,14.5A1.25,1.25 0 0,1 15.5,15.75A1.25,1.25 0 0,1 14.25,17A1.25,1.25 0 0,1 13,15.75A1.25,1.25 0 0,1 14.25,14.5Z" fill="#1e40af"/></svg>
                        </div>
                    </div>
                    <!-- <span class="text-primary font-medium">X% </span>
                    <span class="text-muted-color">since last visit</span> -->
            ...
            <div class="col-span-12 lg:col-span-6 xl:col-span-3">
                <div class="card mb-0">
                    <div class="flex justify-between mb-4">
                        ...
                        <div class="flex items-center justify-center bg-blue-100 dark:bg-blue-400/10 rounded-border" style="width: 2.5rem; height: 2.5rem">
                            <!-- <i class="pi pi-shopping-cart text-blue-500 !text-xl"></i> -->
                             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><title>water-pump</title><path d="M19,14.5C19,14.5 21,16.67 21,18A2,2 0 0,1 19,20A2,2 0 0,1 17,18C17,16.67 19,14.5 19,14.5M5,18V9A2,2 0 0,1 3,7A2,2 0 0,1 5,5V4A2,2 0 0,1 7,2H9A2,2 0 0,1 11,4V5H19A2,2 0 0,1 21,7V9L21,11A1,1 0 0,1 22,12A1,1 0 0,1 21,13H17A1,1 0 0,1 16,12A1,1 0 0,1 17,11V9H11V18H12A2,2 0 0,1 14,20V22H2V20A2,2 0 0,1 4,18H5Z" fill="#1e40af"/></svg>
                        </div>
                    </div>
...
const computedPumpStatus = computed(() => {
  return checked_mode.value
    ? (checked.value ? "On" : "Off") // Kalau manual, pakai toggle user
...
async function fetchModules() {
  try {
    ...
    const nozzleModule = modules.find(m =>
      m.module?.name?.toLowerCase().includes('pump') || m.module?.name?.toLowerCase().includes('nozzle')
    );
...
function generateChartData() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    // Hitung rata-rata
    const avgTemp = tempLogsToday.length
      ? tempLogsToday.reduce((sum, l) => sum + parseFloat(l.context.value), 0) / tempLogsToday.length
      : null;

    const avgHumid = humidLogsToday.length
      ? humidLogsToday.reduce((sum, l) => sum + parseFloat(l.context.value), 0) / humidLogsToday.length
      : null;
...
function connectWebSocket() {
  ...
  ws.onmessage = async (event) => {
    ...
    if (data.event === "pusher:connection_established") {
      ...
      const token = localStorage.getItem("token");
      const authResponse = await fetch("http://localhost:3001/auth.php", {
        method: "POST",
...
watch(() => route.params.garden, async (newModuleId) => {
...
watch(() => route.params.device, async (newGardentId) => {
    loading.value = true; // Tampilkan loading saat mengambil data
    await fetchModules();
    await fetchAnalytics(); // Ambil data device types untuk proyek baru
    loading.value = false; // Sembunyikan loading setelah data diambil
...
watch(checked_mode, async (newVal) => {
  ...
  try {
    await axios.put(
      `/api/gardens/${gardenId}/devices/${deviceId}/settings/${moduleIdNozzle.value}/value`,
      payload,
      {
        headers: {
          Authorization: `Bearer ${token}`
