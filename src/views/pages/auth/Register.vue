            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <span class="text-muted-color font-medium">Sign up to create account</span>
                    </div>

                    <div>
                        <form @submit.prevent="handleRegister">
                        <label for="name" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Name</label>
                        <InputText id="name" type="text" placeholder="Name" class="w-full md:w-[30rem] mb-8" v-model="form.name" />

                        <label for="username" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Username</label>
                        <InputText id="username" type="text" placeholder="Username" class="w-full md:w-[30rem] mb-8" v-model="form.username" />
                        ...
                        <label for="password2" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                        ...
                        <Button label="Sign In" type="submit" class="w-full" as="router-link" to="/dashboard"></Button>
                        </form>
                    </div>
                </div>
            </div>
...
export default {
  ...
  methods: {
    async handleRegister() {
      try {
        const response = await axios.post('https://api.gardenision.com/api/register', this.form);
...
const handleRegister = async () => {
  ...
  loading.value = true
  try {
    const response = await axios.post('https://api.gardenision.com/api/register', {
      name: name.value,
...
<template>
  ...
  <div class="bg-surface-50 dark:bg-surface-950 flex items-center justify-center min-h-screen px-4">
    <div
      class="w-full max-w-md bg-white dark:bg-gray-900 p-8 rounded-2xl shadow-xl"
    ...
    >
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        ...
        <button
          type="submit"
          :disabled="loading"
          class="w-full py-2 px-4 text-white bg-blue-600 hover:bg-blue-700 rounded-lg text-sm font-medium transition disabled:opacity-50"
        >
